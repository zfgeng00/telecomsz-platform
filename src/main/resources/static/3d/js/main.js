...
var mainInit = function (window) {
    ...
    function initEvents() {
        ...
        if (sortByNameCtrl && sortByNameCtrl.length) {
...
/**
 * 移动元素
 * @param obj 必须 需要移动的元素
 * @param ac 必须 true允许，false禁止
 * @param oParent ac为false时不需要 在父元素内移动
 * @param cb ac为false时不需要 移动时的回调方法
 * @returns {boolean}
 */
function drag(obj, ac, oParent, cb) {
    if (!ac) {
    ...
    var l = sent.l || 0;
    var r = sent.r || dmW - obj.outerWidth();
    var t = sent.t || 0;
    var b = sent.b || dmH - obj.outerHeight();

    obj.mousedown(function (ev) {
        var oEvent = ev || event;
        var sentX = oEvent.clientX - obj.offset().left;
        var sentY = oEvent.clientY - obj.offset().top;
...
//                    slideTop = b;
//                }
            obj.offset({left: slideLeft, top: slideTop});
            var vw = $(window).width() / 100
                , vh = $(window).height() / 100
                , vm = vw < vh ? vw : vh;
            var x = parseFloat(obj.css("left")) / vm;
            var y = parseFloat(obj.css("top")) / vm;
            obj.css({
                'left': x + 'vmin',
                'top': y + 'vmin'
            });
